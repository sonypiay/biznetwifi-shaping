        ...
        <div class="uk-grid-medium uk-flex-center" uk-grid>
          <div v-for="device in devices.results" class="uk-width-1-4@xl uk-width-1-4@l uk-width-1-2@m uk-width-1-2@s card-devices">
            <div class="uk-tile uk-tile-default card-device-box">
              ...
              <button @click="deleteDevice(device.USERNAME, device.MAC_ADDRESS)" class="uk-width-1-1 uk-button uk-button-default btn-delete-device">{{ custdash.btndelete }}</button>
            </div>
          </div>
        </div>
      </div>
    ...
    deleteDevice(user, mac)
    {
      swal({
        ...
        if( value ) {
          axios({
            method: 'delete',
            url: this.url + '/biznetwifi/deleteMemberDevice/' + user + '/' + mac
          }).then( res => {
            swal({
              title: 'Berhasil',
